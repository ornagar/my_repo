...
std::shared_ptr<Entry> build_tree(const std::string& name, size_t* count_libs,
                size_t* count_files); //throws bad_alloc, and invalid argument
...
public:
    ...
    void print_tree() const noexcept { print(""); std::cout << KWHT << std::flush; }
...
public:
	explicit File(const std::string& name);//throws bad_alloc
...
{ 
    ...
    p.pop_back();
    p.pop_back();
    std::cout << p << "├──" << Entry::get_name() <<std::endl; 
}
